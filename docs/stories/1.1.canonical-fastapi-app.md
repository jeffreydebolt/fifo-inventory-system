...
## Tasks / Subtasks
- [x] Audit existing FastAPI app modules to capture required features (AC: 1)  
  - [x] Document the routers, middleware, logging, metrics, and diagnostics each variant currently enables (AC: 1)
- [x] Build the canonical FastAPI app file by merging required functionality (AC: 1)  
  - [x] Ensure router inclusion (`runs`, `files`) matches current production surface (AC: 1)  
  - [x] Consolidate observability hooks: Prometheus metrics, Sentry init, logging config (AC: 1)  
  - [x] Preserve health endpoints (`/health`, `/healthz`, `/debug/database`) with consistent responses (AC: 1)
- [x] Retire or proxy alternate app files (AC: 2)  
  - [x] Replace contents with imports to the canonical app or archive them, leaving clear deprecation notice (AC: 2)  
  - [x] Search repository for references to removed modules and update imports accordingly (AC: 2)
- [x] Update documentation describing runtime contexts (AC: 3)  
  - [x] Revise `docs/architecture.md` “Runtime Contexts” section to reflect single entrypoint (AC: 3)  
  - [x] Note the canonical module in deployment notes or README as appropriate (AC: 3)
- [x] Perform local smoke check of canonical app (AC: 1,2)  
  - [x] Start app via `uvicorn api.app:app` and verify `/health`, `/healthz`, `/metrics` return expected responses (AC: 1)
...
- `python3 - <<'PY' ...` (initial runs failed until fastapi and supporting libs installed; see dependency installs above)
- `python3 - <<'PY' ...` (final smoke verification after installing FastAPI stack; `/`, `/health`, `/healthz`, `/metrics` all returned 200)

### Completion Notes List
- Unified FastAPI runtime by keeping `api/app.py` canonical and adding a root route for backward compatibility.
- Replaced legacy entrypoints (`app_simple*`, `app_minimal`) with thin shims importing the canonical app and emitting deprecation warnings; updated `start.py` to import `api.app`.
- Refreshed architecture documentation to describe the single entrypoint and flagged aliases as legacy.
- Installed FastAPI/test dependencies (fastapi, httpx, python-dotenv, sentry-sdk, prometheus-client, supabase, pandas, python-multipart) on system Python 3.13 and confirmed smoke test outputs.

### File List
- api/app.py
- api/app_simple.py
- api/app_simple_production.py
- api/app_minimal.py
- start.py
...
#### Summary
- ✅ Verified that `api/app.py` is the sole configuration hub and exposes routers, diagnostics, and new root heartbeat.
- ✅ Confirmed legacy entrypoints now delegate to the canonical app with clear deprecation warnings; launcher (`start.py`) imports `api.app` directly.
- ✅ Documentation updated to reference the single entrypoint and note shim status.
- ✅ Manual smoke test (`TestClient`) exercised `/`, `/health`, `/healthz`, `/metrics` successfully.

#### Requirements Traceability
- **AC1** → `api/app.py` consolidates routers, metrics, Sentry integration, and new root route.
- **AC2** → `api/app_simple*.py`, `api/app_minimal.py`, and `start.py` import `api.app` and emit deprecation guidance.
- **AC3** → `docs/architecture.md` and `docs/architecture/project-structure.md` now document the canonical entrypoint and legacy aliases.

#### Testing Evidence
- Manual smoke verification via `python3 - <<'PY'` FastAPI `TestClient` covering `/`, `/health`, `/healthz`, `/metrics` (all HTTP 200).

#### Observations & Recommendations
- No functional or documentation gaps detected.
- Recommend notifying deployment owners about the deprecation warnings so downstream tooling can migrate to `api.app` before the shims are removed in a future sprint.
