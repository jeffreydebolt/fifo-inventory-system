...
def process_returns(purchases_df, sales_df, supabase_client, logger):
    ...
    for _, return_row in returns_df.iterrows():
        ...
        try:
            ...
            logger.info(f"✅ Return: {sku} - {return_qty} units @ ${unit_cost:.2f}")
            logger.info(f"   Added to Lot {lot_id}, Negative COGS: ${negative_cogs:.2f}")
        except Exception as e:
            logger.error(f"Failed to process return: {e}")
    
    return regular_sales_df

def validate_sale_dates(purchases_df, sales_df, logger):
    ...
    for _, sale in sales_df.iterrows():
        ...
        if available_before.empty:
            ...
            if not future_lots.empty:
                ...
                warning = (f"SKU {sku}: Sale on {sale_date.strftime('%Y-%m-%d')} "
                          f"for {quantity} units, but earliest inventory is from "
                          f"{earliest_date.strftime('%Y-%m-%d')} ({days_diff} days later)")
                
                date_warnings.append(warning)
    
    if date_warnings:
        logger.warning(f"\n{'='*60}")
        logger.warning("⚠️  DATE VALIDATION WARNINGS:")
        logger.warning(f"{'='*60}")
        
        for warning in date_warnings:
            logger.warning(f"  - {warning}")
        
        logger.warning(f"\nTotal: {len(date_warnings)} warnings")
        logger.warning(f"{'='*60}\n")
    
    return date_warnings

# === End Enhancement Functions ===
# --- Configure Logging ---
logging.basicConfig(
    level=logging.INFO,
