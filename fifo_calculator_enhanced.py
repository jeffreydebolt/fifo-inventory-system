...
# === ENHANCEMENT FUNCTIONS START HERE ===
def process_returns(purchases_df, sales_df, supabase_client, logging):
    """Process returns (negative quantities) before regular sales"""
    returns_df = sales_df[sales_df['Quantity_Sold'] < 0].copy()
    
    if returns_df.empty:
        return sales_df
    
    logging.info(f"\n{'='*60}")
    logging.info(f"Processing {len(returns_df)} returns...")
    logging.info(f"{'='*60}")
    
    for _, return_row in returns_df.iterrows():
        sku = return_row['SKU']
        return_qty = abs(return_row['Quantity_Sold'])
        
        sku_lots = purchases_df[purchases_df['SKU'] == sku].sort_values('Received_Date')
        if not sku_lots.empty:
            oldest_lot = sku_lots.iloc[0]
            unit_cost = oldest_lot['Unit_Price'] + oldest_lot['Freight_Cost_Per_Unit']
            lot_id = oldest_lot['Lot_ID']
            new_remaining = oldest_lot['Remaining_Unit_Qty'] + return_qty
            
            try:
                response = supabase_client.table('purchase_lots').update({
                    'remaining_unit_qty': int(new_remaining)
                }).eq('lot_id', lot_id).execute()
                
                # Update local dataframe too
                purchases_df.loc[purchases_df['Lot_ID'] == lot_id, 'Remaining_Unit_Qty'] = new_remaining
                
                logging.info(f"✅ Return processed: {sku} - {return_qty} units")
                logging.info(f"   Lot {lot_id}: {oldest_lot['Remaining_Unit_Qty']} → {new_remaining}")
                logging.info(f"   Negative COGS: ${-return_qty * unit_cost:.2f}")
            except Exception as e:
                logging.error(f"Failed to process return: {e}")
    
    return sales_df

def validate_sale_dates(purchases_df, sales_df, logging):
    """Validate that inventory exists before sale dates"""
    date_warnings = []
    
    for _, sale in sales_df.iterrows():
        sku = sale['SKU']
        sale_date = pd.to_datetime(sale['Sale_Date'])
        quantity = sale['Quantity_Sold']
        
        available_before = purchases_df[
            (purchases_df['SKU'] == sku) & 
            (purchases_df['Remaining_Unit_Qty'] > 0) &
            (pd.to_datetime(purchases_df['Received_Date']) <= sale_date)
        ]
        
        if available_before.empty:
            future_lots = purchases_df[
                (purchases_df['SKU'] == sku) & 
                (purchases_df['Remaining_Unit_Qty'] > 0)
            ]
            
            if not future_lots.empty:
                earliest_date = pd.to_datetime(future_lots['Received_Date'].min())
                days_diff = (earliest_date - sale_date).days
                date_warnings.append(f"SKU {sku}: Sale on {sale_date.strftime('%Y-%m-%d')} for {quantity} units, but earliest inventory is from {earliest_date.strftime('%Y-%m-%d')} ({days_diff} days later)")
    
    if date_warnings:
        logging.warning(f"\n{'='*60}")
        logging.warning("⚠️  DATE VALIDATION WARNINGS:")
        logging.warning(f"{'='*60}")
        for warning in date_warnings:
            logging.warning(f"  - {warning}")
        logging.warning(f"{'='*60}")
        logging.warning(f"Total: {len(date_warnings)} warnings")
        logging.warning("Note: Calculations continued using available inventory.")
        logging.warning(f"{'='*60}\n")
    
    return date_warnings
# === ENHANCEMENT FUNCTIONS END HERE ===
...
def load_and_validate_sales_user_format(file_path):
    ...
    # === ENHANCEMENT: Don't filter out negative quantities yet ===
...
def main():
    ...
    # === ENHANCEMENT: Process returns and validate dates ===
    df_sales = process_returns(df_purchases, df_sales, supabase, logging)
    date_warnings = validate_sale_dates(df_purchases, df_sales, logging)
    
    # Save date warnings if any
    if date_warnings:
        warnings_file = os.path.join(args.output_dir, 'date_validation_warnings.txt')
        with open(warnings_file, 'w') as f:
            f.write("DATE VALIDATION WARNINGS\n")
            f.write("=" * 60 + "\n")
            f.write(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            for warning in date_warnings:
                f.write(f"{warning}\n")
            f.write(f"\nTotal warnings: {len(date_warnings)}\n")
        logging.info(f"Date warnings saved to: {warnings_file}")
    
    # Filter out negative quantities after processing returns
